...
억지로 계산하려 했던 게 원인이었다 — None을 반환하도록 고쳤다. (3) 동료 리뷰 체크리스트로 견고성을 점검하다가
`graph.py` 전체에 `try/except`가 단 한 줄도 없다는 걸 발견했다 — 코디네이터·점검자 LLM 호출은 `json.loads(raw)`를
그대로 쓰고 있어서, 모델이 JSON 형식을 조금만 어겨도(따옴표 깨짐 등) 그 자리에서 전체 실행이 죽는 구조였다.
`safe_chat_json`/`safe_chat_text`로 감싸 API 에러·JSON 파싱 실패를 한 번 재시도하고, 그래도 안 되면 안전한
기본값(기획 실패 시 코퍼스 앞부분 5건으로 절 1개 구성, 점검 실패 시 "일단 통과") 으로 넘어가면서 실패 이력을
`runtime_errors`에 남기도록 고쳤다 — `chat()`을 monkeypatch해 강제로 실패시키는 오프라인 테스트로 재시도 2회 후
정확히 폴백되는 것도 확인했다.
